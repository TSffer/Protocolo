...
string Protocolo::desencriptar(string mns)
{
    ...
    string Rubrica(aux,0,ele(N)*2);
    string mensaje(aux,ele(N)*2,aux.size()); 
...
string Protocolo::FirmarMensaje(string tPlano)
{
	...
	string Rubrica;
	long long deZZ = to_string(alfabeto.size()-1).size();
	...
	for(int i=0; i<tPlano.size() ;i++)
	{
		aux = alfabeto.find(tPlano[i]);
		auxS = to_string(aux);
		if (auxS.size() < deZZ)
		{
			for (int j=0; j< deZZ - auxS.size() ;j++)
				auxS = "0"+auxS;
		}
		tPlanoZZstring += auxS ;
	}
...
string Protocolo::VerFirma(string tmp)
{
	...
	int deZZ = to_string(alfabeto.size()-1).size();
	string aux1;
	...
	char aux2[deZZ-1];
	long long aa,cont2;
	for(int j=0;j<tDescifrado.size();j+=deZZ)
	{
		tDescifrado_.copy(aux2,deZZ,j);
