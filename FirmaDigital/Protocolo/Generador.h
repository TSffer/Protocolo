...
vector<bool> llenarG(vector<bool> &i_contenedor,int tam_bits)
{
    vector<bool> bb;
    int *aa = seed();
    int k=0;
...
bool tapsG(vector<bool> comodin , int num_tapsG)
{
    bool total;
    for(int i=0;i<num_tapsG;i++)
    {
...
string funcionRotar(string cadena,int tam_bits)
{
    ...
    for(int i=0;i<num_vueltas;i++)
    {
        ...
        for(int p=0;p<num_particiones/2;p++)
        {
            ...
            comodin[indice_1] = tapsG(comodin,num_tapsG);
            indice_1 += largo_bloque;
            comodin[indice_2] = tapsG(comodin,num_tapsG);
...
string funcionG(int tam_bits, int largo_semilla,int num_vueltas,int num_particiones,int num_tapsG)
{
    vector<bool> i_contenedor;
    string Sem;
    i_contenedor.resize(largo_semilla);
    llenarG(i_contenedor,tam_bits);     
...
{
    string Bloques = funcionG(bits,56,15,7,10);
    string GG=Bloques;
    string Num;
